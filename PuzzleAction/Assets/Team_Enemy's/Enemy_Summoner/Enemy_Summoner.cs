...
public class Enemy_Summoner : MonoBehaviour, IEnemyBehaviour
{
    ...
    {
        m_mapGeneration = FindAnyObjectByType<MapGeneration>();
        ...
        if (distance <= m_enemyController.AttackRange)
        {
            ...
            if (m_enemyController.AttackRange > radius)
            {
                for (int i = 0; i < 2; i++)
                {
                    ...
                    {
                        //hitしたObjectが m_mapGeneration の (SouthWall || WestWall) && Active(true)だった場合
                        //if(hit.collider == (m_mapGeneration.SouthWall))
                        {
                            //壁がある場合は召喚位置を変更する
                            //Summon_pos = 召喚者の位置から壁までの距離(Mathf.Max(召喚者の位置, hitした壁の位置) - Mathf.Min(召喚者の位置, hitした壁の位置))　＆＆　AttackRange内にSpawn
                            //Summon_pos = Mathf.Max(Radise,hit.collider)-Mathf.Min(Radise,hit.collider);
                        }
                    }
