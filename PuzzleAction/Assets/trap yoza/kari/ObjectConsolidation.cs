...
{
    public objcari m_ItemManager;
    [Header("四角い部屋の敷き詰め設定")]
    [Tooltip("部屋の横幅(x)と置く幅(z)のサイズ")]
    [SerializeField] public Vector2 m_RoomSize = new Vector2(0f, 0f);
    [Header("【テスト用】インスペクターから選ぶ")]
    [SerializeField]private TrapType m_testTraptype= TrapType.Gas;
    /// <summary>
    ...
    public void DeployTrap(Vector3 spawnPos, TrapType trapType,Vector2 roomSize)
    {
        if (m_ItemManager == null) return;

        string repuestName = trapType.ToString();
        //get pool object
        GameObject spwanedTrap = m_ItemManager.kari(repuestName, spawnPos);
